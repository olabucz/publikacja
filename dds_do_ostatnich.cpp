...
int main() {
    ...
    vector<vector<int>> C_auto_t = wczytaj_tablice_wektorow("APPA_C_autor_od_t.txt"); 
    vector<vector<int>> C_obce_t = wczytaj_tablice_wektorow("APPA_C_obce1_od_t.txt"); 
	...
	if((N_t.size() != C_obce_t.size()) || (N_t.size() != C_auto_t.size()) || (C_obce_t.size() != C_auto_t.size()) ){
		...
		for(int i = 0; i < N_t.size(); i++) //czyli dla każdej osoby ----------- N_t.size()
		{
			...
			//wszystkie cytowania
			string name="APPA_auto1_ost_wszystkie_" + to_string(i+1) + ".txt"; 
			ofstream plik_wynikowy(name);
			
			//cytowania własne
			string name_auto="APPA_auto1_ost_auto_" + to_string(i+1) + ".txt";
			ofstream plik_wynikowy_auto(name_auto);
			
			//cytowania obce
			string name_obce="APPA_auto1_ost_obce_" + to_string(i+1) + ".txt";
			ofstream plik_wynikowy_obce(name_obce);
			
			//czas
			string name_czas="APPA_auto1_ost_czas_" + to_string(i+1) + ".txt"; 
			...
			for (int p = 0; p < 100; p++)
			{
				...
				for (int j = 0; j < C_obce_t[i].size(); j++){ //wstawianie wartosci - N, Cobce, Cauto - z wektorow do tablicy
					...
					//wyliczenie sumy publikacji do j-tego kroku (do pozniejszej dystrybucji cytowan)
					int sum_of_elems = 0;
					for(int s = 0; s < j; s++){
					...
					//przypadek, gdy sumaryczna liczba publikacji do j-tego kroku jest mniejsza niż liczba autocytowan w j-tym kroku
						...
						for(int l = 0; l < C_auto_t[i][j]; l++){
							tablica.push_back(N[N.size() - 1 - 1 - x - xx]);
							cyt_wlasne.push_back(tablica[tablica.size() - 1]);	
							
							if((N.size() - 1 - 1 - xx - x) == 0){ //dotarl do pierwszego elementu (indeks 0) wektora publikacji
								xx +=1;	
