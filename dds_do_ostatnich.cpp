...
int main() {
    ...
    vector<vector<int>> C_auto_t = wczytaj_tablice_wektorow("APPA_C_wspolautorzy_od_t.txt"); 
    vector<vector<int>> C_obce_t = wczytaj_tablice_wektorow("APPA_C_obce3_od_t.txt"); 
	...
	if((N_t.size() != C_obce_t.size()) || (N_t.size() != C_auto_t.size()) || (C_obce_t.size() != C_auto_t.size()) ){
		...
		for(int i = 0; i < N_t.size(); i++) //czyli dla każdej osoby ----------- N_t.size()
		{
			...
			//wszystkie cytowania
			string name="APPA_auto3_ost_wszystkie_" + to_string(i+1) + ".txt"; 
			ofstream plik_wynikowy(name);
			
			//cytowania własne
			string name_auto="APPA_auto3_ost_auto_" + to_string(i+1) + ".txt";
			ofstream plik_wynikowy_auto(name_auto);
			
			//cytowania obce
			string name_obce="APPA_auto3_ost_obce_" + to_string(i+1) + ".txt";
			ofstream plik_wynikowy_obce(name_obce);
			
			//czas
			string name_czas="APPA_auto3_ost_czas_" + to_string(i+1) + ".txt"; 
