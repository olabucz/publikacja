...
int main() {
    ...
    vector<vector<int>> N_t = wczytaj_tablice_wektorow("PRE_N_od_t.txt"); 
    vector<vector<int>> C_auto_t = wczytaj_tablice_wektorow("PRE_C_autorzy_od_t.txt"); 
    vector<vector<int>> C_obce_t = wczytaj_tablice_wektorow("PRE_C_obce2_od_t.txt"); 
	...
	if((N_t.size() != C_obce_t.size()) || (N_t.size() != C_auto_t.size()) || (C_obce_t.size() != C_auto_t.size()) ){
		...
		for(int i = 0 ; i < N_t.size(); i++) //czyli dla każdej osoby ----------- N_t.size()
		{
			...
			//wszystkie cytowania
			string name="PRE_auto2_ost_wszystkie_" + to_string(i+1) + ".txt"; 
			ofstream plik_wynikowy(name);
			
			//cytowania własne
			string name_auto="PRE_auto2_ost_auto_" + to_string(i+1) + ".txt";
			ofstream plik_wynikowy_auto(name_auto);
			
			//cytowania obce
			string name_obce="PRE_auto2_ost_obce_" + to_string(i+1) + ".txt";
			ofstream plik_wynikowy_obce(name_obce);
			
			//czas
			string name_czas="PRE_auto2_ost_czas_" + to_string(i+1) + ".txt"; 
			...
			for (int p = 0; p < 100; p++)
			{
				...
				int sum_of_elems = 0; //suma liczb publikacji
				...
				for (int j = 0; j < C_obce_t[i].size(); j++){ //wstawianie wartosci - N, Cobce, Cauto - z wektorow do tablicy
					...
					//wyliczenie sumy publikacji do j-tego (a wlasciwie j-1) kroku (do pozniejszej dystrybucji autocytowan)
					if((j > 0) && (j <= N_t[i].size())){ //od j = 1 (bo sprawdzamy sumę publikacji do poprzedniego kroku) i do końca 
						sum_of_elems += N_t[i][j-1];	//jeśli jest w zakresie to dodaj liczbę publikacji z poprzedniego roku			
					}
					else{
						sum_of_elems += 0; //jeśli nie, to nic nie dodawaj
					}
